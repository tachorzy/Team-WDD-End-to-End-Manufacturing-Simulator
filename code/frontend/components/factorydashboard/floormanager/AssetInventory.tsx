...
const AssetInventory: React.FC<AssetInventoryProps> = ({ assets, setSelectedAsset, selectedAsset }) => {
    ...
    return (
        <div className="asset-inventory relative z-10 w-full h-[55%] overflow-y-scroll text-white text-lg flex flex-row flex-wrap border-2 border-[#D7D9DF] border-solid">
            {assetList.length > 0 ? (
                assetList.map((asset) => (
                    <AssetItem key={asset.id} asset={asset} setSelectedAsset={setSelectedAsset} selectedAsset={selectedAsset} />
                ))
            ) : (
