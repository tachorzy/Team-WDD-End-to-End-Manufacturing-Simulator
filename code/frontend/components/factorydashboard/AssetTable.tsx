...
const AssetTable: React.FC<{ factoryId: string }> = ({ factoryId }) => {
  ...
  return (
    <div className="p-4">
      <table className="table-auto w-full text-gray-600 bg-white">
        ...
        <tbody>
          {currentAssets.map((asset, index) => (
            <tr key={asset.assetId} className={`border-t border-gray-200 ${index % 2 === 0 ? 'bg-gray-100' : ''}`}>
              <td className="px-4 py-2">
                <img src={asset.imageData || "/icons/floorplan/placeholder-asset.svg"} alt="Asset" className="w-20 h-20 object-cover" />
              </td>
              <td className="px-4 py-2 text-DarkBlue font-semibold break-words">{asset.name}</td>
              <td className="px-4 py-2 text-[#494949] text-xs break-words">Description: {asset.description}</td>
              <td className="px-4 py-2">{asset.factoryId}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <button 
        onClick={() => currentPage < Math.ceil(assets.length / assetsPerPage) && paginate(currentPage + 1)}
        className="px-4 py-2 border rounded bg-white text-black"
