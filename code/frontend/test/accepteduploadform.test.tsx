...
const mockReaderResult = jest.spyOn(global.FileReader.prototype, "result", "get");

const props = {
    uploadedFile: new File(["blobpart"], "image.jpg", { type: `image.jpg` }),
    setUploadedFile: mockSetUploadedFile,
    acceptedFileItems: [
        <li key="image.jpg">image.jpg - 8 bytes</li>,
    ],
    fileRejectionItems: [
        <li key="image.jpg">
            image.jpg - 8 bytes
...
describe("AcceptedUploadForm", () => {
    ...
    test("should render with no errors", () => {
        const { getByAltText, getByTestId, getByText } = render(<AcceptedUploadForm {...props} />);
    ...
    test("should log error when factoryID is missing", async () => {
        ...
        await waitFor(() => {
            expect(consoleSpy).toHaveBeenCalledWith("File or Factory ID is missing.");
        });
    });
    ...
    test("should log error when base64image is missing", async () => {
        ...
        await waitFor(()=> {
            expect(consoleSpy).toHaveBeenCalledWith("Failed to convert the file to base64.");
        });
    }); 
    ...
    test("should log error when createFloorplan fails", async () => {
        ...
        await waitFor(()=> {
            expect(consoleSpy).toHaveBeenCalledWith("Error uploading floor plan:", new Error("404"));
        });
    });
    ...
    test("should log error when there is an error reading the file", async () => {
        ...
        await waitFor(()=> {
            expect(consoleSpy).toHaveBeenCalledWith("There was an error reading the file");
        });
    });
});
