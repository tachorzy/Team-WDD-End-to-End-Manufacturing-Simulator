...
describe("FloorManager", () => {
  beforeEach(() => {
    ...
    (assetAPI.createAsset as jest.Mock).mockImplementation((asset) =>
      Promise.resolve({
        ...
        message: "floorplanId mockGeneratedAssetId created successfully", 
      })
    );
  });
  ...
  test("should add new asset to FloorManager when AddAssetForm is filled out and submitted", async () => {
    ...
    fireEvent.change(descriptionInput, { target: { value: "New Asset Description" } });

    fireEvent.click(createAssetButton);

    await waitFor(() => {
      expect(assetAPI.createAsset).toHaveBeenCalledWith(expect.objectContaining({
        name: "New Asset",
