...
describe('AssetInventory', () => {
  ...
  test('should render list of assets', () => {
    ...
    assets.forEach(asset => {
      const assetNameElement = getByText((content, node) => {
        return node!.textContent === `Name: ${asset.name}`;
      });
      const assetDescriptionElement = getByText((content, node) => {
        return node!.textContent === `Description: ${asset.description}`;
      });
