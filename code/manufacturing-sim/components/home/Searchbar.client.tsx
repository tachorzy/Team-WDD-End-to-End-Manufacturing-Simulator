...
const Searchbar: React.FC<SearchProps> = ({ onSearch }) => {
  ...
  return (
    <div className="mb-8 w-[45%]">
      ...
      {invalidInput && 
        <span id="invalidCoords" className="flex flex-row gap-x-0.5 text-red-400 font-semibold text-xs mt-0.5">
          <Image src="/icons/searchbar/map-error.svg" width={25} height={25} alt="error pin"/>
          <p className="pt-0.5">{"Invalid latitude or longitude provided. Latitude must be between -90° and 90°. Longitude must be between -180° and 180°."}</p>
        </span>
      }
    </div>
  );
