...
const FactoryTable = () => {
  ...
  function getSortedArray(arrayToSort) {
    return sort.direction === "asc"
      ? [...arrayToSort].sort((a, b) => (a[sort.key] > b[sort.key] ? 1 : -1))
      : [...arrayToSort].sort((a, b) => (a[sort.key] < b[sort.key] ? 1 : -1));
  }
            ...
            {tableHeaders.map((header) => (
              ...
              >
                <div className="flex flex-row gap-x-0 align-bottom">
                  ...
                  {sort.direction !== "" && (
                    <span>
                      <Caret direction={sort.direction}></Caret>
                    </span>
                  )}
                </div>
              </th>
          ...
          {facilities.length > 0 ? (
            getSortedArray(facilities).slice(0, 6).map((facility) => (
              <tr key={facility.factoryId} className="text-sm text-[#858A8F]">
                {tableHeaders.map((header) => {
                  let cellValue;
                  if (header.id === "lat" || header.id === "lon") {
                    cellValue =
                      facility.location[
                        header.id === "lat" ? "latitude" : "longitude"
                      ];
                  } else {
                    cellValue = facility[header.id];
                  }
                  return (
                    <td key={header.id} className="border px-4 py-2.5">
                      {cellValue}
                    </td>
                  );
                })}
